...
describe('Technologies Component', () => {
  it('renders section title and description', () => {
    ...
    expect(
      screen.getByText(/Leveraging industry-leading frameworks/)
    ).toBeInTheDocument();
  });
  ...
  it('shows Enterprise Ready label on tech cards in filtered domain view', () => {
    render(<Technologies />);

    const cloudTab = screen.getByRole('tab', { name: 'Cloud & DevOps' });
    fireEvent.click(cloudTab);

    expect(screen.getAllByText('Enterprise Ready').length).toBeGreaterThan(0);
  });
});
